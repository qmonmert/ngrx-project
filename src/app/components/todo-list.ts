    ...
    template: `  
        ...
        <ul>
            <li class="margin-t-20" *ngFor="let todo of todosModel.filteredTodos">
                ...
                <button class="pure-button pure-button-primary" (click)="toggleTodo.emit(todo.id)">
                    {{todo.complete ? 'Undo' : 'Complete'}}
                </button>
                <button class="pure-button button-error" (click)="removeTodo.emit(todo.id)">
                    Remove
                </button>
            </li>
        </ul>
...
export class TodoList {
    ...
    @Output() 
    toggleTodo: EventEmitter<number> = new EventEmitter<number>();
